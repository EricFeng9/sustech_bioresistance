...
/**
 * 破伤风杆菌耐药性数据管理器
 * 负责存储和管理全局的破伤风杆菌对甲硝唑的耐药性数据
 */
public class TetanusResistanceManager extends PersistentState {
    
    // 耐药性数据标识
    private static final String RESISTANCE_KEY = "tetanus_resistance";
    // 每次使用甲硝唑增加的耐药性百分比（0.1% - 0.001f）
    private static final float RESISTANCE_INCREMENT = 0.001f;
    // 当前破伤风杆菌对甲硝唑的耐药性（0.0-1.0）
     ...
     * 创建一个新的耐药性管理器实例
     */
    public TetanusResistanceManager() {
        super();
    }
     ...
     * @return 加载的管理器实例
     */
    public static TetanusResistanceManager createFromNbt(NbtCompound nbt) {
        TetanusResistanceManager manager = new TetanusResistanceManager();
        manager.resistance = nbt.getFloat(RESISTANCE_KEY);
        return manager;
    }
    
    /**
     * 获取当前破伤风杆菌的耐药性（0.0-1.0）
     * @return 当前耐药性值
     */
    ...
    public boolean setResistance(float value) {
        ...
        try {
            ...
            Bioresistance.LOGGER.error("设置破伤风杆菌耐药性失败: {}", e.getMessage());
            return false;
        }
    }
    
    /**
     * 获取当前破伤风杆菌的耐药性（百分比格式，0-100%）
     * @return 耐药性百分比字符串
     */
     ...
     */
    public static final PersistentState.Type<TetanusResistanceManager> TYPE = 
        new PersistentState.Type<>(
            () -> new TetanusResistanceManager(),
            TetanusResistanceManager::createFromNbt,
            null
        );
     ...
     * @return 耐药性管理器实例
     */
    public static TetanusResistanceManager getManager(MinecraftServer server) {
        // 从服务器的主世界持久状态管理器获取数据
